...
class EEGVisualizer:
    ...
    def start_visualization(self):
        ...
        self.animation = FuncAnimation(
            self.fig, self._update_plot, interval=100, blit=False  # Update every 100ms
        )
    ...
    def _update_plot(self, frame):
        ...
        try:
            ...
            for i, ch_name in enumerate(self.channels):
                alpha_idx = np.logical_and(
                    self.freq_vector >= 8, self.freq_vector <= 12
                )
            ...
            if self.headset._is_connected:
                self.status_text.set_text(
                    f"Status: Connected - Recording Time: {int(time.time() - self.headset._recording_start_time)}s"
                )
