...
class TirednessClassifier:
    ...
    def __init__(self, sfreq, channels=['O1', 'O2'], alert_threshold=1.0, tired_threshold=3.0):
        """
    ...
    def get_tiredness_percentage(self, data_chunk):
        ...
        theta_power = np.mean(psd[:, (freqs >= self.THETA_BAND[0]) & (freqs < self.THETA_BAND[1])])
...
def estimate_tiredness_ignoring_noise():
    ...
    try:
        file_paths = [os.path.join(TARGET_DATA_FOLDER, f) for f in os.listdir(TARGET_DATA_FOLDER) if f.endswith('_raw.fif')]
        if not file_paths:
    ...
    print("WARNING: Artifact rejection is disabled. Results will be affected by noise.\n")
    ...
    if not all_percentages:
        ...
        print("Reason: Could not calculate any ratios. The data may be completely flat or corrupted.")
    else:
        ...
        print("Because noise rejection was disabled, it is primarily a measure of how much low-frequency")
        print("noise (from movement, muscle tension, or bad electrode contact) is in the recording.")
